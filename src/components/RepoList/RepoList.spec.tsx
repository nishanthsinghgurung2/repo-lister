...
const testSetup = (UPDATED_MOCK_REPOS?: any) => {
  const { container } = render(
    <MockedProvider
      mocks={UPDATED_MOCK_REPOS ? UPDATED_MOCK_REPOS : MOCK_REPOS}
    >
      <RepoList />
    </MockedProvider>
  );

  const searchInputText: HTMLInputElement = getByTestId(
    container,
    'repo-search-text'
  ) as HTMLInputElement;
  fireEvent.change(searchInputText, { target: { value: 'tennis' } });

  const searchButton: HTMLButtonElement = getByTestId(
    container,
    'repo-search-btn'
...
describe('RepoList', () => {
  ...
  it('should show the repo lists when repo title is entered and search button clicked ', async () => {
    ...
    const repoUrlTextElement = await findByText(
      repoUrlElement,
  ...
  it('should show 10 repos in the current page', async () => {
    const container = testSetup();
    const repoListContainerIUlElement = await findByTestId(
      container,
      'repo-list-container'
    );
  ...
  it('show validation error if empty search title is submitted', async () => {
    const container = testSetup();
    ...
    fireEvent.change(searchInputText, { target: { value: '  ' } });
    ...
    const repoUrlElement = await findByTestId(
      container,
      'repo-search-validation-error'
    );
    expect(repoUrlElement).toBeTruthy();
  });
  ...
  it('should show error message when api fails to fetch response', async () => {
    ...
    const repoListErrorElement = await findByTestId(
      container,
      'repo-list-fetch-error'
    );
  ...
  it('should show warning message when api fetches empty response', async () => {
    ...
    const repoListEmptyWarningElement = await findByTestId(
      container,
      'repo-list-empty-warning'
    );
  ...
  describe('Pagination', () => {
    it('should have pagination', async () => {
      const container = testSetup();
      const repoListPaginationElement = await findByTestId(
        container,
    ...
    it('should load page 2 repos when clicked on that pagination button', async () => {
      ...
      const paginationElement = await findByTestId(
        container,
        'repo-list-pagination'
      );
      const paginationPage2Button: HTMLButtonElement = paginationElement.querySelector(
    ...
    it('should load next page repos when clicked on that pagination button', async () => {
      ...
      const paginationNextPageButton: HTMLButtonElement = paginationElement.querySelector(
    ...
    it('should load previous page repos when clicked on that pagination button', async () => {
      ...
      const paginationPrevPageButton: HTMLButtonElement = paginationElement.querySelector(
